            ...
            <div>
                ...
                <select name="pasien_id" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-teal-500 outline-none text-sm">
                    ...
                    <?php foreach($pasien as $p): ?>
                        <option value="<?= $p['id'] ?>"><?= htmlspecialchars($p['nama_pasien']) ?> (<?= htmlspecialchars($p['no_identitas']) ?>)</option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="penyakit_id" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-teal-500 outline-none text-sm">
                    ...
                    <?php foreach($penyakit as $py): ?>
                        <option value="<?= $py['id'] ?>"><?= htmlspecialchars($py['nama_penyakit']) ?> [<?= htmlspecialchars($py['kode_icd']) ?>]</option>
                    <?php endforeach; ?>
                </select>
            </div>
            
            <!-- Menggunakan relasi tabel Dokter -->
            <div>
                <label class="block text-xs font-bold text-gray-700 mb-1">Dokter Penanggung Jawab</label>
                <select name="dokter_id" required class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-teal-500 outline-none text-sm">
